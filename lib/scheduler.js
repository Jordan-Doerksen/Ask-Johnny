...
async function closeTriviaPoll(ctx, channel, msg, p) {
  ...
  if (fetchFailed) {
    await channel.send({
      embeds: [ctx.embeds.verdictEmbed({ title: '🧠 trivia', description: `it was ${answer}. reactions didn't load right, so no points this round.` })],
    });
    return;
  }
  ...
  const desc = winners.length
    ...
    : `it was ${answer}. nobody got it. bleak.`;
